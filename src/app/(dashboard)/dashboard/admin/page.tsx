...
function AlertItem({ count, label, href, severity }: AlertItemProps) {
  const severityClass =
    severity === "warn" ? "text-amber-700" : "text-blue-700";
  return (
    <Link
      href={href}
    ...
    >
      <span className="text-slate-700">
        <span className={`font-semibold ${severityClass}`}>{count}</span>{" "}
        {label}
      </span>
...
export default async function AdminOverviewPage() {
  ...
  const hasAlerts = unassignedStudents > 0 || classesWithoutAdviser > 0;

  return (
    <>
      <DashboardPageHeader
        title={firstName ? `Good day, ${firstName}` : "Admin overview"}
      />
